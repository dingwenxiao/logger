...
/**
 * Manage k8s connection. This class wrap all the methods that try to connect k8s, and once the
 * microservice failed to connect the k8s, it will retry connecting n times.
 * 
 * @author dixiao
 *
 */
public class KubernetesConnectionUtils {
   ...
   */
  public synchronized static EndpointsList getEndPointsListBylabels(KubernetesClient client,
    ...
    while (retryNum > 0 && endPointsList == null) {
      try {
        if (client != null && client.endpoints() != null) {
